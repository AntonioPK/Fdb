...
# Based on original Perl script by yoyo - http://mersenneforum.org/
...
do{
	...
	do{
		(my $sec,my $min,my $hour,my $mday,my $mon,my $year,my $wday,my $yday,my $isdst) = localtime();
		my $contents = get("http://factorization.ath.cx/listtype.php?t=3&mindig=$mindig&perpage=$numtoget{'current'}&start=$rand&download=1");
		++$waitrequests;
